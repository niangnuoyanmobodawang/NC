...
public class Speed extends Module {
 ...
 //   private double lastDist2;
    ...
    public Property<speedMode> mode = new Property<speedMode>(this, "Mode", speedMode.NEWHOP);
    ...
    public enum speedMode{
        NEWHOP , OLD, HOP, SLOWHOP,  YPORT, TRENDE, LONGHOP, LATEST, MINEPLEX;
    }
    ...
    public void onPre(EventPreMotionUpdates e){
        ...
        if (mode.value.equals(speedMode.NEWHOP)){
           double forward = mc.thePlayer.posX - mc.thePlayer.prevPosX;
           double strafe = mc.thePlayer.posZ - mc.thePlayer.prevPosZ;
           this.lastDist = Math.sqrt(forward * forward + strafe * strafe);
       }
    ...
    public void onMove(EventMove event){
        ...
        if(mode.value.equals(speedMode.NEWHOP)){
            ...
            if (stage != 2 || !mc.thePlayer.isCollidedVertically || mc.thePlayer.moveForward == 0.0F && mc.thePlayer.moveStrafing == 0.0F) {
               if (stage == 3) {
                  forward = (float) (0.66D * (this.lastDist - getBaseMoveSpeed()));
               ...
               em.setMotionY(mc.thePlayer.motionY = 0.4D);
               this.moveSpeed *= 1.533D;
            }

            this.moveSpeed = Math.max(this.moveSpeed, getBaseMoveSpeed());
            if (stage > 0) {
               forward = (float)mc.thePlayer.movementInput.moveForward;
               strafe = (float)mc.thePlayer.movementInput.moveStrafe;
               ...
               if (forward == 0.0D && strafe == 0.0D) {
                  ...
                  em.setMotionX(forward * this.moveSpeed * Math.cos(Math.toRadians((double)(yaw + 90.0F))) + strafe * this.moveSpeed * Math.sin(Math.toRadians((double)(yaw + 90.0F))));
                  em.setMotionZ(forward * this.moveSpeed * Math.sin(Math.toRadians((double)(yaw + 90.0F))) - strafe * this.moveSpeed * Math.cos(Math.toRadians((double)(yaw + 90.0F))));
               }
            }
    ...
    public void doHops(EventMove event, float forward, float strafe, double mx, double mz) {
        ...
        if ((level == 1) && ((mc.thePlayer.moveForward != 0.0F) || (mc.thePlayer.moveStrafing != 0.0F))) {
            ...
            if (mc.thePlayer.isCollidedVertically) {
                if (!mode.value.equals(speedMode.YPORT))
                    moveSpeed *= s ? 1.50001 : 2.149D;
                    mc.thePlayer.motionY = 0.4050001;
                event.y = 0.405;
                if (mode.value.equals(speedMode.YPORT)) {
                    moveSpeed *= 2.3875D;
                    Timer.timerSpeed = 1.05F;
