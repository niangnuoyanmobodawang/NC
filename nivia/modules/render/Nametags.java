...
public class Nametags extends Module {
	...
	public Property<Boolean> Hitrange = new Property<Boolean>(this, "Hitrange", false);
	...
	public Property<Boolean> font = new Property<Boolean>(this, "Font", false);
	...
	public DoubleProperty scale = new DoubleProperty(this, "Scale", 10, 1, 20);
	public DoubleProperty entities = new DoubleProperty(this, "Entities", 50, 10, 150);

	public void drawSphere(double red, double green, double blue, double alpha, double x, double y, double z,
	...
	public void onRender3D(Event3D render) {
		...
		if(Hitrange.value){
		for (final EntityPlayer en : (List<EntityPlayer>) Helper.mc().theWorld.playerEntities) {
			...
			float range = 3.5f;
			if (FriendManager.isFriend(en.getName())) {
				drawSphere(0.0, 1, 1, 0.5, xPos, yPos, zPos, range, lines, lines, 2);
				continue;
			}
			if (mc.thePlayer.getDistanceToEntity(en) >= range) {
				if (mc.thePlayer.isOnSameTeam(en)) {
					drawSphere(0.5, 1, 0.5, 0.5, xPos, yPos, zPos, range, lines, lines, 2);
				} else {
					drawSphere(1, 0.8, 0.4, 0.5, xPos, yPos, zPos, range, lines, lines, 2);
				}

			} else {
				if (mc.thePlayer.isOnSameTeam(en)) {
					drawSphere(1, 0.4, 0.6, 0.7, xPos, yPos, zPos, range, lines, lines, 2);
				} else {
					drawSphere(1, 0.6, 0.4, 0.7, xPos, yPos, zPos, range, lines, lines, 2);
				}
			}
    ...
    public static String getHealthColor(final float percent) {
        if (percent >= 0.9) {
            return "§a";
        }
        if (percent >= 0.75) {
	...
	private void drawNames(EntityPlayer player) {
		...
		if(health.value) {
			...
			drawString( player.isSneaking() ? "§5" : "" +  getHealthColor(player.getHealth() / player.getMaxHealth())+(MathUtils.round((double)(player.getHealth() / 2.0f), 1) + "❤"), w - getWidth(getHealth(player) + " ") , 0, color);
		}
		if(ping.value)
	...
	private void renderEnchantText(final ItemStack stack, final int x, final int y) {
		...
		if (stack.getEnchantmentTagList() != null && stack.getEnchantmentTagList().tagCount() >= 6) {
			Helper.get2DUtils().drawStringWithShadow("[小心]", x * 2, enchantmentY, 16711680);
			return;
		}
