...
public class NiviaHUD extends HUDMode {
	...
	public void renderWatermark() {
		...
		mc.fontRendererObj.drawStringWithShadow("Edited-b" + Pandora.getClientVersion(), mc.fontRendererObj.getStringWidth("Nivia") + (t ? 20 : 15), 3, 0xFF00CCFF);
		GlStateManager.popMatrix();
	}
	...
	public void renderCoordinates() {
        ...
        if(gui.values.value) {
			...
			Helper.get2DUtils().drawStringWithShadow("Time: " + EnumChatFormatting.WHITE + h+":"+mi, x1, y2, gui.getTabGUIColor());
     ...
     //   Logger.logChat((x1 - 150) +"|"+ (y2 - 62));
        String ms = (n > 10) ? ("¡ì2" + n) : ("¡ì4" + n);
        if(Pandora.getModManager().getModState("Scaffold"))
        Helper.get2DUtils().drawStringWithShadow(ms, Helper.get2DUtils().scaledRes().getScaledWidth() / 2 - Helper.get2DUtils().getStringWidth(ms) + 4, Helper.get2DUtils().scaledRes().getScaledHeight() / 2 - 10,gui.getTabGUIColor());
	...
	public void renderPotionStatus() {
        ...
        for (final PotionEffect effect : (Collection<PotionEffect>) this.mc.thePlayer.getActivePotionEffects()) {
            ...
            switch (effect.getAmplifier()) {
                case 1:
                    PType = PType + EnumChatFormatting.DARK_AQUA + " II(¶þ)";
                    break;
                case 2:
                    PType = PType + EnumChatFormatting.BLUE +  " III(Èý)";
                    break;
                case 3:
                    PType = PType + EnumChatFormatting.DARK_PURPLE+  " IV(ËÄ)";
                    break;
                default:
                    break;
            }
