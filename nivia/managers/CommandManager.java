...
public class CommandManager {
	...
	public void call(EventPacketSend packet){
		if (packet.getPacket() instanceof C01PacketChatMessage) {
            ...
            if (packetChatMessage.getMessage().startsWith("-") || packetChatMessage.getMessage().contains("-bind")) {
                for (final Command command : cmds) {
                	//Pandora.prefix
           //     	if(packetChatMessage.getMessage().equals("-") || packetChatMessage.getMessage().equals("-help")){
          //              Logger.logChat("-bind xxx");
             //           packet.setCancelled(true);
            //    	}
                    if (packetChatMessage.getMessage().split(" ")[0].equalsIgnoreCase("-" + command.getName())) {
                        try {
                        ...
                        for (final String alias : command.getAliases()) {
                            if (packetChatMessage.getMessage().split(" ")[0].equalsIgnoreCase("-" + alias)) {
                                try {
                                    command.execute(packetChatMessage.getMessage(), packetChatMessage.getMessage().split(" "));
                                    if(command.isModuleCommand()) Pandora.getFileManager().saveFiles();
                                }
