...
import nivia.modules.render.XRAY;
...
public class Block
{
    ...
    {//System.out.println("test");
        ...
        if (Helper.xray && Helper.containsID(getIdFromBlock(this))) {
        	synchronized(this){
        		boolean casnx = side == EnumFacing.DOWN && this.minY > 0.0D ? true : (side == EnumFacing.UP && this.maxY < 1.0D ? true : (side == EnumFacing.NORTH && this.minZ > 0.0D ? true : (side == EnumFacing.SOUTH && this.maxZ < 1.0D ? true : (side == EnumFacing.WEST && this.minX > 0.0D ? true : (side == EnumFacing.EAST && this.maxX < 1.0D ? true : !worldIn.getBlockState(pos).getBlock().isOpaqueCube())))));
            if(Helper.DIF && casnx && this == Blocks.diamond_ore ){
            	try{
            		try{
            			//	synchronized(this){
            				Helper.dimblock.add(this);
            			//	}
            		//		renderTracer(pos.getX(),pos.getY(),pos.getZ());
            		//		dimblock.add(pos);
            			}catch(Exception x){x.printStackTrace();}
                 Logger.logChat("×êÊ¯:X:"+pos.getX()+"|Y:"+pos.getY()+"|Z:"+pos.getZ());
            	}catch(Exception xz){xz.printStackTrace();}
            }
