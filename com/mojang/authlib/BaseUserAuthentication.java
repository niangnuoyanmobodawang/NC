   ...
   public void loadFromStorage(Map credentials) {
      ...
      if(credentials.containsKey("userProperties")) {
         try {
            ...
            while(t.hasNext()) {
               Map propertyMap = (Map)t.next();
               String propertyMap1 = (String)propertyMap.get("name");
               name = (String)propertyMap.get("value");
               value = (String)propertyMap.get("signature");
               if(value == null) {
                  this.getModifiableUserProperties().put(propertyMap1, new Property(propertyMap1, name));
               } else {
                  this.getModifiableUserProperties().put(propertyMap1, new Property(propertyMap1, name, value));
               }
            }
      ...
      if(credentials.containsKey("displayName") && credentials.containsKey("uuid")) {
         ...
         if(credentials.containsKey("profileProperties")) {
            try {
               ...
               Iterator propertyMap2 = t1.iterator();

               while(propertyMap2.hasNext()) {
                  Map propertyMap3 = (Map)propertyMap2.next();
                  name = (String)propertyMap3.get("name");
                  value = (String)propertyMap3.get("value");
                  String signature = (String)propertyMap3.get("signature");
                  if(signature == null) {
   ...
   public Map saveForStorage() {
      ...
      if(!this.getUserProperties().isEmpty()) {
         ...
         while(properties.hasNext()) {
            Property userProperty = (Property)properties.next();
            HashMap profileProperty = new HashMap();
            profileProperty.put("name", userProperty.getName());
            profileProperty.put("value", userProperty.getValue());
            profileProperty.put("signature", userProperty.getSignature());
      ...
      if(selectedProfile1 != null) {
         ...
         Iterator userProperty1 = selectedProfile1.getProperties().values().iterator();

         while(userProperty1.hasNext()) {
            Property profileProperty1 = (Property)userProperty1.next();
